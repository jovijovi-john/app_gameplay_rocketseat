...
// "https://discord.com/invite/n66KFjRZ"
...
export function AppointmentDetails() {
  ...
  async function fetchGuildWidget(){
    ...
    try {
      // informações do servidor (algumas funcionalidades só bot consegue executar)
      const response = await api.get(`/guilds/${guildSelected.guild.id}/widget.json`)
      setWidget(response.data);

    } catch(error) {
      alert("Verifique as configurações do servidor. Será que o widget está habilitado?");
    } finally {
      setLoading(false);
    }
  }
